 ...
 * 代码生成工具初始化（单表全量代码生成）
 */
public class MbgInitApp {
	
	/**
	 * 配置文件
	 */
	private static String configFile= "/generatorInitConfig.xml";
    ...
    public static Configuration getConfiguration(String filePath, ConfigurationParser cp) throws Exception {
        ...
        try {
        	...
        	if(file.exists()) {
        		...
        		in = MbgInitApp.class.getResourceAsStream(filePath);
        	}
        	if(in==null) {
        		String fileNewPath=MbgInitApp.class.getResource("/").getPath()
